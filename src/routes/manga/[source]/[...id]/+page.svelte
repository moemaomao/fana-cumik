<script lang="ts">
	...
	let viewMode = $state<'grid-thumb' | 'grid-text' | 'list-thumb' | 'list-text'>('grid-text');
	...
	let altTitle = $derived(meta['alttitle'] || '');
	...
	let synopsis = $derived(
		(manga.description || '')
			.split(/\n+/)
			.filter((line) => !/^\s*(alttitle|type|language|artists?|groups?|pages|author)\s*:/i.test(line))
			.join('\n')
			.trim()
	);
	...
	function statusClass(status: string) {
		...
		if (s.includes('completed') || s.includes('complete')) return 'text-purple-400';
		if (s.includes('ongoing')) return 'text-emerald-400';
		if (s.includes('dropped')) return 'text-rose-500';
		...
		<div class="relative z-10 p-5 sm:p-8">
			...
			<div class="flex flex-col gap-6 md:flex-row md:gap-8">
				...
				<div class="mx-auto flex w-[150px] shrink-0 flex-col items-center gap-3 sm:w-[170px] md:mx-0 md:w-[190px]">
					...
					<div class="flex items-center gap-1 text-xs font-semibold text-zinc-400">
					...
					<button
						type="button"
						class="flex w-full items-center justify-center gap-2 rounded-xl border border-white/20 bg-white/10 py-2 text-xs font-medium text-white transition hover:bg-white/20"
					>
				...
				<div class="min-w-0 flex-1">
					...
					<!-- Alternative Title (Dibawah Judul Utama) -->
					{#if altTitle}
					...
					<!-- Metadata List (Urutan Diselaraskan Sesuai SS) -->
					<div class="mt-4 grid gap-2 text-xs text-zinc-300 sm:text-sm">
						<!-- 1. Status (Di Paling Atas) -->
						<div class="flex items-center gap-2">
							<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="shrink-0 text-zinc-400"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>
							<span class="font-bold uppercase tracking-wider {statusClass(manga.status)}">
						...
						{#if type}
							<div class="flex items-center gap-2">
								<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="shrink-0 text-zinc-400"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>
								<p><span class="text-zinc-400">Type:</span> <span class="capitalize text-zinc-200">{type}</span></p>
							</div>
						{/if}
						...
						{#if manga.chapters?.length}
							<div class="flex items-center gap-2">
								<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="shrink-0 text-zinc-400"><path d="M4 19.5v-15A2.5 2.5 0 0 1 6.5 2H20v20H6.5a2.5 2.5 0 0 1 0-5H20"/></svg>
								<p><span class="text-zinc-400">Chapter:</span> <span class="text-zinc-200">{manga.chapters.length} Chapters</span></p>
							</div>
						{/if}
						...
						{#if language}
							<div class="flex items-center gap-2">
								<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="shrink-0 text-zinc-400"><circle cx="12" cy="12" r="10"/><path d="M12 2a14.5 14.5 0 0 0 0 20 14.5 14.5 0 0 0 0-20"/><path d="M2 12h20"/></svg>
								<p><span class="text-zinc-400">Language:</span> <span class="text-zinc-200">{language}</span></p>
							</div>
						{/if}

						<!-- 5. Group / Serialization -->
						{#if groups}
							<div class="flex items-center gap-2">
								<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="shrink-0 text-zinc-400"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
								<p><span class="text-zinc-400">Group:</span> <span class="text-zinc-200">{groups}</span></p>
							</div>
						{/if}
						...
						{#if author}
							<div class="flex items-center gap-2">
								<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="shrink-0 text-zinc-400"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
								<p><span class="text-zinc-400">Author:</span> <span class="text-zinc-200">{author}</span></p>
							</div>
						{/if}
						...
						{#if artists}
							<div class="flex items-center gap-2">
								<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="shrink-0 text-zinc-400"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>
								<p><span class="text-zinc-400">Artist:</span> <span class="text-zinc-200">{artists}</span></p>
							</div>
						{/if}

						<!-- 8. Pages / Latest Update -->
						{#if pages}
							<div class="flex items-center gap-2">
								<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="shrink-0 text-zinc-400"><path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"/><path d="M14 2v4a2 2 0 0 0 2 2h4"/></svg>
								<p><span class="text-zinc-400">Pages:</span> <span class="text-zinc-200">{pages} Pages</span></p>
							</div>
						{/if}
					</div>
				</div>
			...
			{#if manga.genres?.length}
				<div class="mt-6 flex flex-wrap gap-2">
					{#each manga.genres as genre}
						<span
							class="rounded-lg border border-white/10 bg-black/40 px-3.5 py-1.5 text-xs text-zinc-300"
						>
							{genre}
						</span>
					{/each}
			...
			{#if synopsis}
				<section class="mt-6 rounded-xl border border-white/10 bg-black/30 p-5">
					<h2 class="mb-3 text-base font-semibold text-white">Synopsis</h2>
					<p class="text-xs leading-relaxed text-zinc-300 sm:text-sm whitespace-pre-line">
						{synopsis}
					</p>
				</section>
			{/if}
			...
			<div class="mt-10 flex items-center gap-3">
				...
				<!-- Section Chapters (Tengah dengan divider) -->
